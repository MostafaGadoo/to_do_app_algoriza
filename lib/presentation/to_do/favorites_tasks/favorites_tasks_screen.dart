...
class FavoritesTasksScreen extends StatelessWidget {
  const FavoritesTasksScreen({Key? key}) : super(key: key);
    ...
    return BlocConsumer<AppCubit,AppStates>(
      ...
      builder: (context, state){
        ...
        return ListView.separated(
          ...
          itemBuilder: (context,index) => TasksWidget(data: cubit.todoFavoritesTasks[index]),
          separatorBuilder: (context,index) => Container(
            height: 1,
            color: Colors.grey,
            width: double.infinity,
          ),
          itemCount: cubit.todoFavoritesTasks.length,
        );
      },
    );
  }
